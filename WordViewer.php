...
foreach($context::DictDB as $t => $d)
{
	if($d == $context->GetDict())
	{
		...
		for($u = 0; $u < count($uses); $u++)
		{
			...
			$WordsOnLine = SQLQuarry(' SELECT   `word`, `id`  FROM `'.$context::BookDB[$t].'`  WHERE `lineNumber` = '.$uses[$u]['lineNumber'].' and `chapter` '.($uses[$u]['chapter'] == "" ? "IS NULL" : "=" ) ." ". $uses[$u]['chapter'].' and `book` = '.$uses[$u]['book'].' ORDER BY `OrderOfText` ASC  ', false ,"id" );
